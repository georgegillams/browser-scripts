...
function generateReadmeTableColumns (data, dataKeyMap) {
  ...
  for (let columnI = 0; columnI < dataMapKeys.length; columnI += 1) {
    ...
    columns.push({columnName: columnName, columnWidth: columnName.length });
  }
...
function generateReadmeTable (data, dataKeyMap) {
  ...
  for (let columnI = 0; columnI < columns.length; columnI += 1) {
    const columnName = columns[columnI].columnName;
    const columnWidth = columns[columnI].columnWidth;
  ...
  for (let dataI = 0; dataI < data.length; dataI += 1) {
    ...
    result += ` ${pad(CODE_TEXT, `[${CODE_TEXT}](${githubFileLink(codeFileName)})`, codeColumnWidth)} |`
    ...
    if(!greasyId) {
      ...
      result += ` ${pad(GREASY_FORK_TEXT, `[${GREASY_FORK_TEXT}](${greasyForkLink(greasyId)})`, greasyColumnWidth)} |`
    }
  }
...
function githubFileLink (fileName) {
  ...
  return `https://github.com/georgegillams/browser-scripts/blob/master/src/${fileName.split(' ').join('%20')}`;
}
...
if(readmeContentSplit.length > 0) {
  const generatedReadme = `${readmeContentSplit[0]}## Scripts\n\n${generateReadmeTable(data, dataKeyMap)}`;
