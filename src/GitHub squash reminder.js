...
function addReminder() {
  ...
  const commitCount = parseInt(document.getElementById('commits_tab_counter').textContent, 10);
  if (commitCount !== 1) {
    ...
    for (let i = 0; i < allElements.length; i += 1) {
      const element = allElements[i];
      if (element.textContent && element.textContent.includes('Merge pull request')) {
        element.disabled = true;
        element.textContent = 'MERGE ONLY ONCE SQUASHED!';
        element.id = 'squash_reminder_button';
        element.style.backgroundImage = 'linear-gradient(-180deg, #e02626 0%, #9F2D27 90%)';
